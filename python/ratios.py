...
#			(self.h2_ElpRatio   , self.h_ElpRatio_pt   , self.h_ElpRatio_eta   , self.h_ElpRatio_nv   , self.ElpRatio   , self.ElpRatioE   ) = self.calculateRatio(el_samples, 'EE', 'ZDecay')
	...
	def save_ratio_plot(self, pl, path, h_data, h_mc, name = '', x_var = '', y_var = '', x_range = None) :
		...
		histos['mc'  ] = h_mc
		helper.mkdir(path)
		helper.save_histo2table(histos = histos, processes = ['data', 'mc'], path = '%s/%s.dat' % (path, name), var = '%s_%s' % (y_var, x_var), lumi = pl.lumi, bin_width = False, last_bin = False, x_range = x_range)
