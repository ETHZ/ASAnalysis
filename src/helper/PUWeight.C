...
double PUWeight::weightOOT( int npv_in_time, int npv_m50nsBX ){
  //Calculate Weight
  CalculateWeight();
  weightOOT_init();
  ...
  int bin = fWeight->GetXaxis()->FindBin( npv_in_time );

  double inTimeWeight = fWeight->GetBinContent( bin );
