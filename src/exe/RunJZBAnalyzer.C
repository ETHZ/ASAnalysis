...
void usage( int status = 0 ) {
	cout << "Usage: RunJZBAnalyzer [-o filename] [-v verbose] [-n maxEvents] [-t type] [-l] file1 [... filen]" << endl;
	cout << "  where:" << endl;
	...
	cout << "     maxEvents number of events to process          " << endl;
        cout << "     type     is 'data' (default) or 'mc'           " << endl;
...
// Parse options
	char ch;
	while ((ch = getopt(argc, argv, "o:v:n:t:lh?")) != -1 ) {
		switch (ch) {
	...
	JZBAnalyzer *tA = new JZBAnalyzer(theChain,(type=="data"));
