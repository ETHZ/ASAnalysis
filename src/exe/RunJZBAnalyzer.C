...
void usage( int status = 0 ) {
  cout << "Usage: RunJZBAnalyzer [-o filename] [-a analysis] [-v verbose] [-n maxEvents] [-j JSON] [-t type] [-c] [-l] [-p data_PileUp] [-P mc_PileUP] file1 [... filen]" << endl;
  cout << "  where:" << endl;
...
int main(int argc, char* argv[]) {
  ...
  // Parse options
  char ch;
  while ((ch = getopt(argc, argv, "o:v:n:j:t:lh?cp:P:a:")) != -1 ) {
    switch (ch) {
