...
void usage( int status = 0 ) {
	cout << "Usage: RunJZBAnalyzer [-o filename] [-v verbose] [-n maxEvents] [-l] file1 [... filen]" << endl;
	cout << "  where:" << endl;
...
// Parse options
	char ch;
	while ((ch = getopt(argc, argv, "o:v:n:lh?")) != -1 ) {
		switch (ch) {
//			case 'o': outputfile = TString(optarg); break;
	...
	JZBAnalyzer *tA = new JZBAnalyzer(theChain);
