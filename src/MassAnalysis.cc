...
void MassAnalysis::Analyze(){	
	...
	// multijet
	//double run[21]   = {148822, 148829, 148862, 148862, 148864, 148864, 148953, 149011, 149011, 149011, 149181, 149181, 149181, 149181, 149291, 149291, 148864, 148864, 148864, 149058, 149181};
	//double lumi[21]  = {440, 244, 289, 497, 460, 514,  49,  67, 424, 461, 257, 305, 707, 941, 436, 539,  10,  47, 275,  64, 985};
	//double event[21] = {454994856, 239439139, 434297633, 729330524, 529426262, 586943934,  70626194, 105062641, 619833331, 668579778,  87276054, 157837774, 670139805, 925116042, 466605433, 569274059,   9986520,  56130889, 325562876,  76943429, 970118251};
	
	// LM1
	//double event[50] = {84407, 84410, 84411, 84413, 84415, 84429, 84431, 84434, 84437, 84442, 84443, 84444, 84445, 84447, 84452, 84455, 84458, 84460, 84461, 84464, 84465, 84470, 84472, 84484, 84497, 84500, 84504, 84523, 84524, 84527, 84531, 84534, 84547, 84553, 84562, 84568, 84576, 84577, 84579, 84591, 84594, 84595, 84604, 84605, 84614, 84623, 84625, 84629, 84633, 84635};
...
void MassAnalysis::ControlPlots(){
	...
	// NJets
	fHNJets -> Fill(fJetsLoose.size());
	if(fIsCleanMultiJetEvent){
		fHNJets_clean -> Fill(fJetsLoose.size());
	}
	...
	// Jet Pt spectrum
	for(int i=0; i<fJetsLoose.size(); ++i){
		fHJpt  -> Fill(fTR->PFJPt[fJetsLoose[i]]);
		fHJEta -> Fill(fTR->PFJEta[fJetsLoose[i]]);
		if(fIsCleanMultiJetEvent){
			fHJpt_clean  -> Fill(fTR->PFJPt[fJetsLoose[i]]); 
			fHJEta_clean -> Fill(fTR->PFJEta[fJetsLoose[i]]);
		}
	}
...
void MassAnalysis::PseudoJetMasses(){
	...
	// fill tree variable
	fMT2tree->misc.PseudojetAlphaT = alphaT;
	...
	for(int i=0; i<fJetsLoose.size(); ++i){
		px.push_back(fTR->PFJPx[fJetsLoose[i]]);
		py.push_back(fTR->PFJPy[fJetsLoose[i]]);
		pz.push_back(fTR->PFJPz[fJetsLoose[i]]);
		 E.push_back(fTR->PFJE[fJetsLoose[i]]);
	}
	...
	for(int i=0; i<fMT2_histos_number; ++i){
		...
		if(fBJets.size()>0)    {fHMT2_PseudoJetWithB[i]       -> Fill(MT2);}
			...
			fHPseudoJetMT2vsLeadingJEta   -> Fill(MT2, fTR->PFJEta[fJetsLoose[0]]);
		...
		if(i==0){
			// fill tree variable
			fMT2tree->misc.PseudoJetMT2 = MT2;
			if(pseudojet1.Pt() > pseudojet2.Pt()){
				fMT2tree->misc.PseudoJet1Pt = pseudojet1.Pt();
				fMT2tree->misc.PseudoJet2Pt = pseudojet2.Pt();
				fMT2tree->pseudoJets[0].SetPxPyPzE(pseudojet1.Px(), pseudojet1.Py(),
								 pseudojet1.Pz(), pseudojet1.E ());
				fMT2tree->pseudoJets[1].SetPxPyPzE(pseudojet2.Px(), pseudojet2.Py(),
								 pseudojet2.Pz(), pseudojet2.E ());
			} else {
				fMT2tree->misc.PseudoJet1Pt = pseudojet2.Pt();
				fMT2tree->misc.PseudoJet2Pt = pseudojet1.Pt();
				fMT2tree->pseudoJets[1].SetPxPyPzE(pseudojet1.Px(), pseudojet1.Py(),
								 pseudojet1.Pz(), pseudojet1.E ());
				fMT2tree->pseudoJets[0].SetPxPyPzE(pseudojet2.Px(), pseudojet2.Py(),
								 pseudojet2.Pz(), pseudojet2.E ());
			}		
		}	
	}
	...
	fHPseudoJetMT2AxisdPhi -> Fill(vec1.Angle(vec2));	
...
void MassAnalysis::MultiBMasses(){
	...
	vector<float> px, py, pz, E;
	for(int i=0; i<fJetsLoose.size(); ++i ){
		px.push_back(fTR->JPx[fJetsLoose[i]]);
		py.push_back(fTR->JPy[fJetsLoose[i]]);
		pz.push_back(fTR->JPz[fJetsLoose[i]]);
		E.push_back(fTR->JE[fJetsLoose[i]]);
	}
	...
	for(int i=0; i<fJetsLoose.size(); ++i){
		if(grouping[i]==1 && fTR->JbTagProbSimpSVHighPur[fJetsLoose[i]] > 2.0) b_group1.push_back(fJetsLoose[i]);
		if(grouping[i]==2 && fTR->JbTagProbSimpSVHighPur[fJetsLoose[i]] > 2.0) b_group2.push_back(fJetsLoose[i]);
	}
...
void MassAnalysis::DiJetMasses(){
	if( fJetsLoose.size()!=2 ) return;    // only look at dijets
	...
	TLorentzVector p1;
	TLorentzVector p2;		
	p1.SetPxPyPzE(fTR->PFJPx[fJetsLoose[0]], fTR->PFJPy[fJetsLoose[0]], fTR->PFJPz[fJetsLoose[0]], fTR->PFJE[fJetsLoose[0]]);
	p2.SetPxPyPzE(fTR->PFJPx[fJetsLoose[1]], fTR->PFJPy[fJetsLoose[1]], fTR->PFJPz[fJetsLoose[1]], fTR->PFJE[fJetsLoose[1]]);
...
void MassAnalysis::SingleLeptonMasses(){
	...
	if(fJetsLoose.size()!=0) return; // plots also MT in case there are no selected jets (pure W -> l nu selection..)
...
void MassAnalysis::DiLeptonMasses(){
	...
	for(int i=0; i<fJetsLoose.size(); ++i){
		TLorentzVector p;
		p.SetPxPyPzE(fTR->PFJPx[fJetsLoose[i]],fTR->PFJPy[fJetsLoose[i]],fTR->PFJPz[fJetsLoose[i]],fTR->PFJE[fJetsLoose[i]]);
		P_UTM +=p;
	}
...
void MassAnalysis::PrintEvent(){
	...
	cout << endl;
	for(int i=0; i<fJetsLoose.size(); ++i){
		cout << "Jet" << i+1 << ": Pt " << fTR->PFJPt[fJetsLoose[i]]  
		     << " Phi " << fTR->PFJPhi[fJetsLoose[i]] << " Eta " << fTR->PFJEta[fJetsLoose[i]] <<endl;
	}
	for(int i=0; i<fElecs.size(); ++i){
